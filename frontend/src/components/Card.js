...
function Card(props) {
  ...
  return (            
    <li className="place">
    <img src={removeIcon} alt="удалить" className={`place__remove ${currentUser.userId === props.cardObj.owner ? '' : 'place__remove_hidden'}`} onClick={handleDelete} />
    ...
    <div className="place__subscription">
      ...
      <div className="place__likes">
        <button type="button" className={`place__like ${props.cardObj.likes.some(id => id === currentUser.userId)? "place__like_active" : ''}`} onClick={handleLike}></button>
